...
export default function AdminEventNew() {
  ...
  async function submit() {
    ...
    try { const id = await createEvent({ name: name.trim(), event_date: date, course_id: courseId || null, participant_code: participantCode.trim() || null, playerIds: selectedPlayers, questions: questions.map((q, i) => ({ question_type_id: q.id, display_order: i + 1, parameters: q.parameters })) }); navigate(`/admin/events/${id}`) } catch (e: any) { setError(e.message ?? 'Tapahtuman luonti epäonnistui') } finally { setSaving(false) }
  }
