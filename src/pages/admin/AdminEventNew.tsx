...
const isHeadToHead = (question: PickedQuestion) => question.key === 'yes_no_head_to_head'
const needsTargetPlayer = (question: PickedQuestion) => question.requires_target_player || question.key === 'beat_the_leader'
...
export default function AdminEventNew() {
  ...
  const valid = name.trim() && selectedPlayers.length >= 2 && questions.length >= 4 && questions.length <= 8 && (!participantCode.trim() || (participantCode.trim().length >= 2 && participantCode.trim().length <= 20)) && questions.every(q => (!needsTargetPlayer(q) || (q.parameters.player_id && selectedPlayers.includes(String(q.parameters.player_id)))) && (!isHeadToHead(q) || (q.parameters.player_a_id && q.parameters.player_b_id && q.parameters.player_a_id !== q.parameters.player_b_id && selectedPlayers.includes(String(q.parameters.player_a_id)) && selectedPlayers.includes(String(q.parameters.player_b_id)))))
  ...
  async function submit() {
    ...
    try { const id = await createEvent({ name: name.trim(), event_date: date, course_id: courseId || null, participant_code: participantCode.trim() || null, playerIds: selectedPlayers, questions: questions.map((q, i) => ({ question_type_id: q.id, question_type_key: q.key, question_text: q.display_name, points_possible: q.max_points, display_order: i + 1, parameters: q.parameters })) }); navigate(`/admin/events/${id}`) } catch (e: any) { setError(e.message ?? 'Tapahtuman luonti epäonnistui') } finally { setSaving(false) }
  }
  return <div className="max-w-5xl space-y-7">
    ...
    <section className="card p-5 space-y-2"><h2 className="text-white font-bold mb-3">Valitut kysymykset</h2>{questions.map((q, i) => <div key={q.id} draggable onDragStart={() => setDragged(i)} onDragOver={e => e.preventDefault()} onDrop={() => { if (dragged != null) move(dragged, i); setDragged(null) }} className="flex flex-wrap items-center gap-3 border-b border-white/5 py-3 last:border-0"><span className="text-gray-500 cursor-grab">⠿</span><span className="text-xs text-gray-500 w-5">{i + 1}</span><span className="text-white flex-1">{q.display_name}</span>{needsTargetPlayer(q) && <select value={String(q.parameters.player_id ?? '')} onChange={e => setQuestions(current => current.map((item, index) => index === i ? { ...item, parameters: { ...item.parameters, player_id: e.target.value } } : item))} className="input text-xs"><option value="">Valitse kohdepelaaja</option>{players.filter(p => selectedPlayers.includes(p.id)).map(p => <option key={p.id} value={p.id}>{p.full_name}</option>)}</select>}{isHeadToHead(q) && <><select value={String(q.parameters.player_a_id ?? '')} onChange={e => setQuestions(current => current.map((item, index) => index === i ? { ...item, parameters: { ...item.parameters, player_a_id: e.target.value } } : item))} className="input text-xs"><option value="">Pelaaja A</option>{players.filter(p => selectedPlayers.includes(p.id)).map(p => <option key={p.id} value={p.id}>{p.full_name}</option>)}</select><select value={String(q.parameters.player_b_id ?? '')} onChange={e => setQuestions(current => current.map((item, index) => index === i ? { ...item, parameters: { ...item.parameters, player_b_id: e.target.value } } : item))} className="input text-xs"><option value="">Pelaaja B</option>{players.filter(p => selectedPlayers.includes(p.id)).map(p => <option key={p.id} value={p.id}>{p.full_name}</option>)}</select></>}<button type="button" onClick={() => setQuestions(current => current.filter((_, index) => index !== i))} className="text-gray-500 hover:text-red-300">✕</button></div>)}{questions.length === 0 && <div className="text-sm text-gray-500">Valitse kysymyksiä yllä.</div>}</section>
