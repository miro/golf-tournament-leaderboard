...
export default function AdminEventNew() {
  ...
  const valid = name.trim() && selectedPlayers.length >= 2 && questions.length >= 4 && questions.length <= 8 && (!participantCode.trim() || (participantCode.trim().length >= 2 && participantCode.trim().length <= 20)) && questions.every(q => (!q.requires_target_player || (q.parameters.player_id && selectedPlayers.includes(String(q.parameters.player_id)))) && (!isHeadToHead(q) || (q.parameters.player_a_id && q.parameters.player_b_id && q.parameters.player_a_id !== q.parameters.player_b_id && selectedPlayers.includes(String(q.parameters.player_a_id)) && selectedPlayers.includes(String(q.parameters.player_b_id)))))
