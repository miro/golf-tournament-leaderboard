...
export default function AdminEventNew() {
  ...
  const valid = name.trim() && selectedPlayers.length >= 2 && questions.length >= 4 && questions.length <= 8 && questions.every(q => !q.requires_target_player || (q.parameters.player_id && selectedPlayers.includes(String(q.parameters.player_id))))
  ...
  async function submit() {
    ...
    try { const id = await createEvent({ name: name.trim(), event_date: date, course_id: courseId || null, playerIds: selectedPlayers, questions: questions.map((q, i) => ({ question_type_id: q.id, display_order: i + 1, parameters: q.parameters })) }); navigate(`/admin/events/${id}`) } catch (e: any) { setError(e.message ?? 'Tapahtuman luonti epäonnistui') } finally { setSaving(false) }
  }
